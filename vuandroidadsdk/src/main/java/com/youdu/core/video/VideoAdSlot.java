...
public class VideoAdSlot implements ADVideoPlayerListener {
    ...
    private void pauseVideo(boolean isAuto) {
        if (mVideoView != null) {
            ...
            mVideoView.seekAndPause(0);//这里和业务逻辑相关，当划出屏幕50%的时候，回到0去暂停，根据业务逻辑自行改变
        }
    }
    ...
    public void updateAdInScrollView() {
        ...
        //1.小于0表示未出现在屏幕上，不做任何处理
        if (currentArea <= 0) {
            return;
        }
        //2.刚要滑入和滑出时，异常状态的处理
        //这种情况是为了排除，假如我们的videoView放在listview里面的item以后，
        //我们的item View在刚刚要划出屏幕的时候，他的面积会突然从零变成100，所以需要把这种异常情况排除掉
        if (Math.abs(currentArea - lastArea) >= 100) {
            return;
        }
        //3.滑动没有超出屏幕50%时走入该case
        if (currentArea < SDKConstant.VIDEO_SCREEN_PERCENT) {
            //进入自动暂停状态
            if (canPause) {
                pauseVideo(true);
                canPause = false;//事件的过滤
            }
            lastArea = 0;
    ...
    private void backToSmallMode(int position) {
        ...
        mVideoView.seekAndResume(position);//是播放器调到指定位置并且播放
